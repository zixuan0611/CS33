			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	...
	#pragma omp parallel for num_threads(28) private(j, index_X, index_Y)
   	for(i = 0; i<n; i++){
   		for(j = 0; j < Ones; j++){
...
void func2(double *weights, double *probability, int n)
{
	...
	#pragma omp parallel for num_threads(28) reduction(+:sumWeights)
   	for(i = 0; i < n; i++)
...
void func3(double *arrayX, double *arrayY, double *weights, double *x_e, double *y_e, int n)
{
	...
	#pragma omp parallel for num_threads(28) reduction(+:estimate_x, estimate_y) 
	for(i = 0; i < n; i++){
...
{
	int i;
	double lianxiao = (double)n;

	#pragma omp parallel for num_threads(28) firstprivate(n, lianxiao)
	for(i = 0; i < n; i++){
   		u[i] = u1 + i/lianxiao;
   	}
}
...
void func5(double *x_j, double *y_j, double *arrayX, double *arrayY, double *weights, double *cfd, double *u, int n)
{
	...
	#pragma omp parallel for num_threads(28) private(i)
	for(j = 0; j < n; j++){
